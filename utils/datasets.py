    ...
    def __init__(self, path, img_size=416):
        ...
        if os.path.isdir(path):
            ...
            if path.startswith('https'):
                response = requests.get('https://' + path[7:])
            else:
                response = requests.get('http://' + path[6:])
            img_name = path.split('/')[-1] + '.jpg'
            with open('./data/web/' + img_name, 'wb') as fh:
                print(os.listdir('./data/web'))
                fh.write(response.content)
            files = ['./data/web' + img_name]
